...
void write_jpeg_Y_RGB(struct main_mcu_rgb *p_main_rgb){
    ...
    //On rentre les paramètre dans la structure entete set_XXX
    // Caractèristique de l'image à traiter
    jpeg_set_ppm_filename(p_jpeg, p_main_rgb->ppm_filename);
    jpeg_set_image_height(p_jpeg, p_main_rgb->height);
    jpeg_set_image_width(p_jpeg, p_main_rgb->width);
    jpeg_set_nb_components(p_jpeg, 3);
    
    // Paramètre de l'encodage
    jpeg_set_jpeg_filename(p_jpeg, p_main_rgb->jpeg_filename);
    jpeg_set_sampling_factor(p_jpeg, Y, H, 1);
    jpeg_set_sampling_factor(p_jpeg, Cb, H, 1);
    jpeg_set_sampling_factor(p_jpeg, Cr, H, 1);
    jpeg_set_sampling_factor(p_jpeg, Y, V, 1);
    ...
    jpeg_set_huffman_table(p_jpeg, AC, Y, p_main_rgb->htable[1]);
    jpeg_set_huffman_table(p_jpeg, DC, Cb, p_main_rgb->htable[2]);
    jpeg_set_huffman_table(p_jpeg, AC, Cb, p_main_rgb->htable[3]);
    jpeg_set_huffman_table(p_jpeg, DC, Cr, p_main_rgb->htable[2]);
    jpeg_set_huffman_table(p_jpeg, AC, Cr, p_main_rgb->htable[3]);
